...
{{ volume.loopback_device.local_file }}:
  cmd.run:
  - name: truncate --size {{ loopback_device.size|default('1G') }} {{ volume.loopback_device.local_file }}
...
/etc/init/cinder-device-setup.conf:
  ...
  - defaults:
    ...
    loopback_device: {{ volume.loopback_device.device_name|default('/dev/loop0') }}
